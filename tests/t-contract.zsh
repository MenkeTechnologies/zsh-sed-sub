#!/usr/bin/env zunit
#{{{                    MARK:Header
##### Purpose: zsh-sed-sub — plugin-contract pins.
...
@test 'entrypoint stem matches plugin directory basename' {
    ...
    # stem of `zsh-sed-sub.plugin.zsh` must equal the parent directory's
    # basename so generated source lines stay copy-pasteable.
    local entry='zsh-sed-sub.plugin.zsh'
...
@test 'entrypoint parses cleanly under zsh -n' {
    run zsh -n "$pluginDir/zsh-sed-sub.plugin.zsh"
    assert $state equals 0
}
